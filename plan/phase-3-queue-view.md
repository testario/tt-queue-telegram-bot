...
- [ ] Открытие Mini App показывает текущую очередь без лишних действий
- [ ] Таймер тикает в реальном времени для активного матча
- [ ] При изменении состояния очереди (через SSE) экран обновляется без перезагрузки
- [ ] Список ищущих игроков отображается корректно
- [ ] Список отыгравших отображается корректно
- [ ] Состояние паузы отображается баннером
