...
- [ ] Кнопка "Ищу соперника" добавляет игрока в поиск и исчезает
- [ ] Игрок в поиске видит кнопку "Отменить поиск"
- [ ] Кнопка "Сыграть с ним" у каждого ищущего игрока (кроме себя) создаёт матч
- [ ] Игрок в активном матче видит кнопку "Нет времени"
- [ ] Форма прямого приглашения открывается и отправляет запрос
- [ ] Приглашённый игрок видит InviteCard и может принять/отклонить
- [ ] Автор приглашения может его отменить
- [ ] Все изменения отражаются через SSE без перезагрузки
- [ ] Ошибки (уже играл, не в поиске и т.д.) показываются пользователю понятно
