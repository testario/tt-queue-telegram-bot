...
- [ ] AdminPanel не отображается для обычных пользователей
- [ ] AdminPanel отображается только для администраторов чата
- [ ] Кнопка "Поставить на паузу" меняет статус и скрывается
- [ ] Кнопка "Продолжить" снимает паузу и обновляет статус через SSE
- [ ] Кнопка "Экстренная пауза" показывается только при активном матче
- [ ] Экстренная пауза и её снятие работают корректно (как в боте)
- [ ] Для опасных действий показывается confirm-диалог
- [ ] Ошибки (403, уже на паузе) отображаются понятным текстом
- [ ] После действий SSE обновляет состояние без перезагрузки
- [ ] PlayerManager показывает список игроков с аватарами
- [ ] Удаление игрока запрашивает подтверждение и убирает его из списка
- [ ] Удалённый игрок пропадает из пикера в DirectMatchModal
